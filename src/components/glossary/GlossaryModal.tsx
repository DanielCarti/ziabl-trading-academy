...
export default function GlossaryModal({ term, locale, onClose }: GlossaryModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6">
      ...
      {/* Modal Dialog */}
      <div className="relative w-full max-w-lg bg-surface border border-surface-border rounded-2xl p-6 sm:p-7 shadow-2xl z-10 animate-scale-in text-left">
        {/* Header */}
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs text-text-muted uppercase tracking-wider">Глоссарий</span>
              </div>
              <h3 className="text-xl font-bold text-text-primary mt-1">{termTitle}</h3>
        ...
        {/* Definition Content */}
        <div className="p-4 rounded-xl bg-surface-light border border-surface-border/60 text-sm text-text-secondary leading-relaxed mb-6 max-h-72 overflow-y-auto">
          {definition}
        </div>

        {/* Footer Actions */}
        <div className="flex flex-col sm:flex-row items-center justify-between gap-3 pt-2 border-t border-surface-border">
          <Link
            href={`/${locale}/glossary`}
            target="_blank"
            rel="noopener noreferrer"
            className="text-xs text-accent hover:text-accent-hover inline-flex items-center gap-1.5 transition-colors font-medium"
          >
          ...
          <button
            onClick={onClose}
            className="btn-primary w-full sm:w-auto px-5 py-2 text-xs font-medium"
          >
            {isRu ? 'Понятно' : 'Got it'}
          </button>
        </div>
