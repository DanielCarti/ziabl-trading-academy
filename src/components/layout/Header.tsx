...
export default function Header() {
  ...
  const [showClockWidget, setShowClockWidget] = useState<boolean>(false);
  ...
  useEffect(() => {
    fetch('/api/cbr')
      ...
      .catch(() => setCbrInfo({ rate: 14.0, date: '2026-09-10', nextMeeting: '2025-10-24' }));
  }, []);
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          ...
          <div className="hidden md:flex items-center gap-2.5">
            ...
            {showCbrWidget && (
              <div
                title={`Ключевая ставка Банка России: ${cbrInfo ? cbrInfo.rate.toFixed(1) : '14.0'}%${cbrInfo?.date ? ` (установлена: ${cbrInfo.date})` : ''}${cbrInfo?.nextMeeting ? `\nСледующее заседание Совета директоров ЦБ: ${cbrInfo.nextMeeting}` : ''}\nОбновляется по официальному календарю решений ЦБ РФ.`}
              ...
              >
                ...
                <span className="font-mono font-bold text-accent">
                  {cbrInfo !== null ? `${cbrInfo.rate.toFixed(1)}%` : '14.0%'}
                </span>
              </div>
            )}
